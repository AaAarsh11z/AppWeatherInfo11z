  ...
  fetch(url)
    ...
    .then(info => {
      ...
      document.getElementById(`w-icon`).src = 
        `https://cdn.weatherbit.io/static/img/icons/${w.weather.icon}.png`;
      ...
      document.getElementById("sunrise").textContent =
        `Sunrise: ${thr}:${tmin<10? `0${tmin}`: `${tmin}`}`;

      var [hr, min] = w.sunset.split(":").map(Number);
      thr = Math.floor(((hr * 60) + min + (5 * 60) + 30)/60);
      tmin = (min + 30)%60;
      document.getElementById("sunset").textContent =
        `Sunset: ${thr}:${tmin<10? `0${tmin}`: `${tmin}`}`;

      document.getElementById("wdesc").textContent =
        `Weather: ${w.weather.description}`;

      document.getElementById("windd").textContent =
        `Wind Dir: ${w.wind_cdir}`;

      document.getElementById("winds").textContent =
        `Wind Speed: ${w.wind_spd} m/s`;
  ...
  fetch(url)
    ...
    .then(info => {
      ...
      for (let i = 0; i < w.data.length; i++) {
        ...
        document.getElementById(`thig${i}`).textContent =
        `${Math.round(w.data[i].high_temp)}c`;
        document.getElementById(`tlow${i}`).textContent =
        `${Math.round(w.data[i].low_temp)}c`;
        document.getElementById(`rhum${i}`).textContent =
        `${w.data[i].rh}%`;
        document.getElementById(`dewp${i}`).textContent =
        `${Math.round(w.data[i].dewpt)}c`;
        document.getElementById(`desc${i}`).textContent =
        `${w.data[i].weather.description}`;
      }
    })
    .catch(err => console.error("Weather fetch error:", err));
}
